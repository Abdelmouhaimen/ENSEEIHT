...
struct
  ...
  let rec string_of_affectable a = 
    ...
    |Acces (a , e) -> (string_of_affectable a)^ " [ " ^ (string_of_expression e )^ "] "
  ...
  and string_of_expression e =
    ...
    | Adresse id -> "& "^id^" "
    |Newtableau (t ,e) -> "( "^"new "^(string_of_type t)^" [ "^(string_of_expression e)^"] " 

    |Initialisation  le -> "{ "^(String.concat ", " (List.map (string_of_expression) le))^"} "
  ...
  let rec string_of_instruction i =
    ...
    | For (t,n1,e1,e2,n2,e3,b) -> "Pour : For "^"( "^(string_of_type t) ^n1 ^" = "^(string_of_expression e1)^" ; "^
                                   "("^(string_of_expression e2)^") ; "^n2^" = "^(string_of_expression e3)^")\n"^
                                   ((List.fold_right (fun i tq -> (string_of_instruction i)^tq) b ""))^"\n"
    | Goto id -> "Goto : GOTO "^id^ "\n" 
    | Etiquette id -> "Etiquette : ETQ "^id^ "\n"
