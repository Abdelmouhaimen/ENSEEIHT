...
public class Rasterizer {
    ...
    public void rasterizeFace (Fragment v1, Fragment v2, Fragment v3) {
        ...
        // Récupération des limites du triangle en x et y
        ...
        for (int x = minX; x <= maxX; x++) {
            for (int y = minY; y <= maxY; y++) {
                ...
                double gamma = 0.0;
                try {
                    barycentricCoords = C.multiply(new Vector3(1, x, y));
                    alpha = barycentricCoords.get(0);
                    beta = barycentricCoords.get(1);
                    gamma = barycentricCoords.get(2);
                ...
                if (alpha >= 0 && beta >= 0 && gamma >= 0) {
                    // Création d'un fragment pour le point (x, y)
                    Fragment fragment = new Fragment(x, y);
                    
                    // Interpolation des attributs du fragment
                    interpolateAttributes(v1, v2, v3, alpha, beta, gamma, fragment);
                    
                    // Appel au shader pour déterminer la couleur du fragment
                    shader.shade(fragment);
                }
            }
        }
    ...
    private void interpolateAttributes(Fragment v1, Fragment v2, Fragment v3, double alpha, double beta, double gamma, Fragment fragment) {
        int numAttributes = v1.getNumAttributes();
        for (int i = 0; i < numAttributes; i++) {
            double interpolatedAttribute = alpha * v1.getAttribute(i) +
                                        beta * v2.getAttribute(i) +
                                        gamma * v3.getAttribute(i);
            fragment.setAttribute(i, interpolatedAttribute);
        }
    }
