...
[C,~,ic] = unique(resultats(:,1),'rows','stable');
h = accumarray(ic,1);
[~,ind] = max(h);
numero_reconnu = C(ind,1);

% Recherche de l'offset temporel le plus commun
offsets = resultats(resultats(:,1) == numero_reconnu, 2);
[C,~,ic] = unique(offsets,'rows','stable');
h = accumarray(ic,1);
[~,ind] = max(h);
meilleur_offset = C(ind,1);


if numero_reconnu == numero_morceau
    fprintf('Le morceau "%s" a ete correctement reconnu avec un décalage temporel de %d !\n', fichiers(numero_morceau), (meilleur_offset/f_ech) * n_decalage);
else
