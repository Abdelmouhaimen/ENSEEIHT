...
function newton(f::Function, gradf::Function, hessf::Function, x0::Union{Real,Vector{<:Real}}; 
   ...
   while(norm(gradf(x_sol))>max(tol_rel*norm(gradf(x0)),tol_abs))
      ...
      if norm(gradf(x_sol)) <= max(tol_rel * norm(gradf(x0)), tol_abs)
         flag = 0  # CN1
         break
      elseif norm(dk) <= epsilon * max(tol_rel * norm(xold), tol_abs)
         flag = 1  # Stagnation de l’itéré 
         break
      elseif abs(f(x_sol) - f(xold)) <= epsilon * max(tol_rel * abs(f(xold)), tol_abs)
         flag = 2  # Stagnation de la fonction
         break
      elseif nb_iters == max_iter # Nb d’itérations max 
         flag = 3
         break
      end
